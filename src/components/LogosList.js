...
export default function LogosList({
	...
	const siblingElements = [].concat.apply([], [children]);
	const db2r = length % 2;
	const db3r = length % 3;
	...
	if (logos.length === 0) {
		return (
			<Tank div className={clsx("LogosList", variation, className)}>
				{title && <h2 className="title">{title}</h2>}
				{tagline && <p className="tagline">{tagline}</p>}
				<div id={id} className={clsx("logos", `cols_${cols}`, `db3r_${db3r}`, `db2r_${db2r}`)}>
					{children}
				</div>
			</Tank>
		);
	...
	return (
		<Tank div className={clsx("LogosList", "animate_switch", list, variation, className)}>
