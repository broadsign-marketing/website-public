...
function Collapse({ title = "", par = "" }: CollapseProps) {
	...
	return (
		<div className={clsx("collapse", isActive ? "active" : "")}>
			<p
...
export default function Inventory__Why({ className = "", elements = [] }) {
	...
	if (isMobile) {
		return (
			<div className={clsx("Inventory__Why bg-zircon rounded-xl overflow-hidden py-4 px-5 mt-5", className)}>
				{elements.map((el, k) => (
	...
	return (
		<div className={clsx("Inventory__Why mt-18", className)}>
			...
			<div className="relative mt-18 bg-zircon rounded-xl">
				{elements.map((el, k) => (
					<div className={clsx("Inventory__Why__par px-12 py-10", String(k) === activeTab ? "active" : "")} key={k}>
