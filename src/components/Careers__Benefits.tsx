...
type FetchedBenefitsList = Array<{ id: string; title: string; description: string }>;
type BenefitBox = {
	id: string;
	title: string;
	description: string;
...
export default function Careers__Benefits({ data }) {
	...
	const perksMobileRef = useRef(null) as unknown as MutableRefObject<HTMLElement>;
	const fetchBenefits = (T?.texts?.benefits?.list as FetchedBenefitsList) || [];

	const screenSize = useScreen();

	const thumbnails = useMemo(
		() => [
			{ type: "img", image: data.benefits_tb_01.childImageSharp.gatsbyImageData } as BenefitBox,
			{ type: "img", image: data.benefits_tb_02.childImageSharp.gatsbyImageData } as BenefitBox,
			{ type: "img", image: data.benefits_tb_03.childImageSharp.gatsbyImageData } as BenefitBox,
			// { type: "img", image: data.benefits_tb_04.childImageSharp.gatsbyImageData } as BenefitBox,
		],
		[]
	);
	...
	const benefits = useMemo((): BenefitsListColumns | BenefitsColumn => {
		...
		if (screenSize === "xs") {
			...
			fetchBenefits.forEach((benefit: BenefitBox) => {
				out.push({ ...benefit, type: "text", icon: getIcon(benefit.id), gradientDirection: Math.floor(Math.random() * 360) });
			});
			return shuffle(out);
		} else {
			...
			fetchBenefits.forEach((benefit: BenefitBox) => {
				...
				out[currentColumn].push({ ...benefit, type: "text", icon: getIcon(benefit.id), gradientDirection: Math.floor(Math.random() * 360) });

				currentColumn++;
			});

			const numOfPicturesToAdd = 4 - (fetchBenefits.length % 4);

			thumbnails.slice(0, numOfPicturesToAdd).forEach((thumbnail: BenefitBox): void => {
				if (currentColumn >= benefitsColumns) {
					currentColumn = 0;
				}
	...
	}, [fetchBenefits, benefitsColumns, screenSize, thumbnails]);

	const handleWindowResize = useCallback(() => {
		const _setSizes = { xs: 1, sm: 3, md: 3, lg: 4, xl: 4, "2xl": 4 };
		if (_setSizes[screenSize] && benefitsColumns !== _setSizes[screenSize]) {
			setBenefitsColumns(_setSizes[screenSize]);
		}
	}, [benefitsColumns, screenSize]);
	...
	if (benefits.length > 0) {
		return (
			<section className={clsx("container", "benefits", "spyable", prefersReducedMotion ? "reduced_motion" : "")}>
				<div className="bg-white z-100">
					<p className="subtitle-1 gradient">{T.translate("benefits.overtitle")}</p>
					<h2 className="h4">{T.translate("benefits.title")}</h2>
					<p className="line-height-180 max-w-800">{T.translate("benefits.par")}</p>
				</div>
				{screenSize === "xs" && (
					<>
						<div className="Grid Row mobile" ref={perksMobileRef}>
							<div className="wrapper">
								{benefits.map(
									(box: BenefitBox, k: number): HTMLDivElement => (
										<div className="box bg-zircon" key={`box_${k}`}>
											<div className="inner">
												<img src={box.icon} className="box_icon mb-5" alt="" />
							...
							{shuffle(thumbnails)
								...
								.map((tb, k) => (
									<div className="benefit_photo_wrapper" key={k}>
										<Img alt="" image={tb.image} className="benefit_photo" />
									</div>
								))}
						</div>
					</>
				...
				{screenSize !== "xs" && (
					<div className="grid desktop">
						{benefits.map((column, k) => {
							...
							return (
								<div key={`bc_${k}`} className={clsx("col-4 lg:col-3", even)}>
									<Tuna shift={even === "even" ? -20 : -80}>
										{column.map((box: BenefitBox, j) => {
											if (box.type === "text") {
												return (
													<div className="box bg-zircon" key={`box_${k}${j}`}>
														<img alt="" src={box.icon} className="box_icon mb-3" />
														<p className="box_title">{box.title}</p>
														<p className="box_description">{box.description}</p>
													</div>
												);
											...
											if (box.type === "img") {
												return <Img key={`box_${k}${j}`} alt="" image={box.image} className="thumbnail" />;
											}
										})}
									</Tuna>
								</div>
