...
export default function QuoteCarousel({ className, quotes = [], title = "", theme = "lightpanel" }) {
	...
	}, [quotesWrapperRef, resizeCarousel]);
	...
	if (quotes.length === 0) {
		console.debug("No quotes provided to QuoteCarousel.");
		return <div className="QuoteCarousel error">No quotes provided...</div>;
	}
	...
	return (
		<StaticQuery
			...
			render={(data) => {
				return (
					<div className={clsx("QuoteCarousel", `theme_${theme}`, className)}>
						...
						<div className="quotes" ref={quotesWrapperRef} style={{ minHeight: `${carouselHeight}px` }}>
							...
							{quotes.map((quote, k) => (
								<blockquote className={clsx(k === pos ? "active" : "")} key={`quote_${k}`}>
