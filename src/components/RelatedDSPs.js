...
export default function DspSideBar({ exclude, ty_page }) {
	...
	return (
		<div className="SideBar">
			<Link activeClass="active" className="anchor_button_free_trial" to="ebookDOOHPlaybookForProgrammaticBuyers">
				<div className="download">
					<h5>Download</h5>
					<img src={icon_playbook} alt="" />
					<p>Everything you need to launch a programmatic DOOH campaign</p>
				</div>
			</Link>
			{ty_page ? <h3>Dive deeper into programmatic digital out-of-home:</h3> : <h5>Read more</h5>}
			<div className="dsps">
				{exclude !== "add" && (
					<div className="dsp_wrapper">
						<Link to={route("MobileDigitalOutOfHome", l)}>
							<img src={add_icon} alt="" />
							<p>Why you should add programmatically digital out-of-home to your mobile campaign</p>
						</Link>
					</div>
				)}
				{exclude !== "buy" && (
					<div className="dsp_wrapper">
						<Link to={route("DigitalOutOfHomeMediaProgrammatically", l)}>
							<img src={buy_icon} alt="" />
							<p>How can I buy digital out-of-home media programmatically ?</p>
						</Link>
					</div>
				)}
				{exclude !== "what" && (
				...
				{exclude !== "include" && (
					<div className="dsp_wrapper">
						<Link to={route("includeDigitalOutOfHome", l)}>
							...
							<p>Why your next programmatic media campaign should include digital out-of-home</p>
						</Link>
					</div>
				)}
			</div>
