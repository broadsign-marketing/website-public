...
function CTABoxSubtitle({ text, color = "white" }) {
	return <p className={clsx("text-18 font-bold line-height-140 sm:text-20", `text-${color}`)}>{text}</p>;
}

const ReflexCTABox__InlineForm = memo(function ReflexCTABox__InlineForm({ text: { title, par, tagline, subtitle }, form }) {
	const BoxElement = memo(({ formUid }) => {
		return (
			<>
				...
				{subtitle && <CTABoxSubtitle text={subtitle} />}
...
const ReflexCTABox__ModalForm = memo(function ReflexCTABox__ModalForm({ type, text }) {
	...
	return (
		<>
			...
			{showModal && (
				<Modal variant="form" className="theme_carolina narrow" onClose={() => setShowModal(false)}>
...
const BlogCTABox = memo(function BlogCTABox({ postId, tags }) {
	...
	if ((hasFormBeenSubmitted("newsletter") || hasFormBeenSubmitted("newsletterInsideOOH")) && !hasSubmittedNewsletter) {
		setHasSubmittedNewsletter(true);
	}

	const newsletterToPromote = type === "inside-ooh" ? "newsletterInsideOOH" : "newsletter";

	switch (type) {
		case "inside-ooh":
		case "newsletter":
			if (!hasSubmittedNewsletter) {
				return <ReflexCTABox__InlineForm text={T.texts.ctaBox[type]} form={newsletterToPromote} />;
			}
