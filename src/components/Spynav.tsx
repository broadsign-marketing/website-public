...
function MobileSpynav({ id, className, sections, spyingSection }: DeviceSpynavProps) {
	...
	return (
		<div id={id} className={clsx("Spynav", "mobile", isOpen ? "active" : "", className)} role="navigation">
			<div className="container">
				<div className="btn_wrapper">
					{sections.map((section) => (
						<button onClick={() => toggleIsOpen()} className={clsx(spyingSection === section.id ? "active" : "")} key={section.id}>
							{section.label}
						</button>
					))}
				</div>
				<ul className={clsx("subnav")}>
					{sections.map((section) => (
						<li className={clsx("spylink", `spylink_${section.id}`, spyingSection === section.id ? "active" : "")} key={section.id}>
...
function DesktopSpynav({ id = "", className = "", sections = [], spyingSection = "" }: DeviceSpynavProps) {
	const selfRef: React.MutableRefObject<HTMLDivElement | null> = useRef(null) as unknown as MutableRefObject<HTMLElement>;

	const repositionSpyBar = useCallback((): void => {
		if (selfRef?.current?.style && typeof document !== "undefined") {
			//const MainPos = Math.max((document.querySelector("main") as HTMLElement).getBoundingClientRect().x, 0);
			//(selfRef.current.style as React.CSSProperties).left = `${MainPos}px`;
		}
	}, [selfRef]);
	...
	return (
		<div id={id} className={clsx("Spynav", "desktop", className)} role="navigation" ref={selfRef}>
			<Container className={`active_${spyingSection}`}>
				{sections.map((section) => (
					<div className={clsx("spylink", `spylink_${section.id}`, spyingSection === section.id ? "active" : "")} key={section.id}>
...
export default function Spynav({ id = "", className = "", sections = [], defaultValue = "", breakpoint = "sm" }: SpynavProps) {
	...
	return (
		<>
			<MobileSpynav id={id} className={className} sections={sections} spyingSection={spyingSection} />
			<DesktopSpynav id={id} className={className} sections={sections} spyingSection={spyingSection} />
		</>
	);
}
