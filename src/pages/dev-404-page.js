...
export default function DevNotFoundPage({ pageContext: { l, dicoPath }, location: { pathname } }) {
	useDico("en", "noLocal");
	...
	useEffect(() => {
		...
		if (process.env.GATSBY_ACTIVE_ENV !== "development" || process.env.NODE_ENV !== "development") {
			navigate("/");
		}
	}, []);
		...
		<Layout id="sitemap">
			<Tank className="table">
				...
				<table>
					<tbody>
						{allRoutes.map((r, k) => {
							const allPaths = Object.values(r).join(",");
							if (allPaths.match(/resources.broadsign.com/)) {
								return null;
							}
