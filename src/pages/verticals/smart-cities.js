...
import "@sass/pages/verticals/smart_cities.scss";

const VerticalSmartCitiesPage = ({ pageContext: { l, dicoPath }, location: { pathname }, data }) => {
	useDico(l, dicoPath);
	...
	return (
		<Layout id="page_vertical_smart_cities" className="theme_carolina">
			...
			<SpielPoster img={data.spielBg.childImageSharp.gatsbyImageData}>
				<h3 className="text-34">A smart city network without limits</h3>
				<p className="text-20">
					Integrate with external data providers and whatever exciting technology you like. Our open, extensible API helps ensure your network's
...
export default VerticalSmartCitiesPage;

export const queryPageImages = graphql`
	query verticalSmartCitiesImages {
		Hero: file(relativePath: { eq: "heroes/vertical_smart_cities_bg.jpg" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
		iconLatticePane1: file(relativePath: { eq: "icons/smart_services.png" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
		...
		spielBg: file(relativePath: { eq: "bg/vertical_smart_cities_spiel_banner_bg.jpg" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
