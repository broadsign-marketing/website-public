...
import "@sass/pages/verticals/outdoor.scss";
...
const VerticalGasStations = ({ pageContext: { l, dicoPath }, location: { pathname }, data }) => {
	...
	const quotes = T.texts.quotes || [];

	return (
		<Layout id="page_vertical_outdoor" className="theme_carolina">
			...
			<SpielPoster className="outdoorSpiel" img={data.spielBg.childImageSharp.gatsbyImageData}>
				<div className="outdoor_spiel_wrapper">
					<h3>{T.translate("SpielPosterOutdoor.title")}</h3>
					<p>{T.translate("SpielPosterOutdoor.text")}</p>
				</div>
			</SpielPoster>
			<QuoteCarousel quotes={quotes} theme="lightpanel" />
...
export const queryPageImages = graphql`
	query verticalOutdoor {
		Hero: file(relativePath: { eq: "heroes/vertical_outdoor_bg.jpg" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
		...
		iconLatticePane4: file(relativePath: { eq: "icons/wayfinding.png" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
		spielBg: file(relativePath: { eq: "bg/vertical_outdoor_spiel_banner_bg.jpg" }) {
			childImageSharp {
				gatsbyImageData
			}
		}
